...
using SmartCommunicationForExcel.Core.Transfer;
...
{
    public class OmronEventIO : ReverseBytesTransform, IEventIO
    {
        public OmronEventIO() : base(DataFormat.CDAB)
        {
            base.IsStringReverseByteWord = true;
        }
        ...
        public bool SetString(string value)
        {
            if (DTType == CDataType.DTString)
            {
                byte[] bbb = Encoding.ASCII.GetBytes(value);
