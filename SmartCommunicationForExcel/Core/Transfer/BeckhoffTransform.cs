...
namespace SmartCommunicationForExcel.Core.Transfer
{
    public class BeckhoffTransform : ByteTransformBase
    {
        public BeckhoffTransform()
        {

        }

        public BeckhoffTransform(DataFormat dataFormat) : base(dataFormat)
        {

        }
        #region Get Value From Bytes

        public override bool[] TransBool(byte[] buffer, int index, int length)
        {
            //Array.Reverse(buffer);
            return base.TransBool(buffer, index, length);
        }
        ...
        public override long TransInt64(byte[] buffer, int index)
        {
            ...
            tmp[2] = buffer[2 + index];
            tmp[3] = buffer[3 + index];
            tmp[4] = buffer[4 + index];
            tmp[5] = buffer[5 + index];
            tmp[6] = buffer[6 + index];
            tmp[7] = buffer[7 + index];
