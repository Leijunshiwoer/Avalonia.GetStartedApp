...
namespace SmartCommunicationForExcel.Core
{
    /// <summary>
    ...
    public class ByteTransformBase : IByteTransform
    {
        ...
        /// <summary>
        /// 从缓存中提取string结果，使用Siemens 字符串格式
        /// </summary>
        /// <returns>string对象</returns>
        public virtual string TransString(byte[] buffer)
        {
            if (buffer.Length > 2)
            {
                byte len0 = buffer[0];//理论长度
                byte len1 = buffer[1];//实际长度
                if (len0 > buffer.Length -2)
                {
                    throw new Exception($"字符串解析异常:string自定义长度:{buffer.Length - 2},理论长度:{len0},数据源:{Encoding.ASCII.GetString(buffer)}");
                }
                
                if(len1 > len0)//plc端存在string互转导致理论长度变化
                {
                    //throw new Exception($"字符串解析异常:string自定义长度:{buffer.Length - 2},实际有效长度:{len1},数据源:{Encoding.ASCII.GetString(buffer)}");
                }
                byte[] tmp = TransByte(buffer, 2, len1);
        ...
        /// <summary>
        /// 从缓存中提取string结果，使用Siemens W字符串格式
        /// </summary>
        /// <param name="buffer"></param>
        /// <returns></returns>
        public string TransWString(byte[] buffer)
        {
            if (buffer.Length > 4)
            {
                int len = buffer[2] + buffer[3] << 8;
                byte[] tmp = TransByte(buffer, 4, len);
        ...
        //暂时没起作用
        public string GetGBKString(byte[] buffer)
        {
            if (buffer.Length > 4)
            {
                byte len0 = buffer[0];
                if (len0 != buffer.Length - 4)
                {
                    throw new Exception($"字符串解析异常:wstring自定义长度:{buffer.Length - 4},实际长度:{len0},数据源:{Encoding.ASCII.GetString(buffer)}");
                }
                byte len1 = buffer[1];
                if (len1 > len0)
                {
                ...
                byte[] tmp = TransByte(buffer, 2, len1);
                return Encoding.GetEncoding("GBK").GetString(tmp);
            }
            else
                return string.Empty;
        }
        ...
        protected byte[] ByteTransDataFormat8(byte[] value, int index = 0)
        {
            ...
            switch (DataFormat)
            {
                ...
                case DataFormat.BADC:
                    {
                        buffer[0] = value[index + 4];
                        buffer[1] = value[index + 5];
                        buffer[2] = value[index + 6];
                        buffer[3] = value[index + 7];
                        buffer[4] = value[index + 0];
                        buffer[5] = value[index + 1];
                        buffer[6] = value[index + 2];
                        buffer[7] = value[index + 3];
                        break;
                    }
                ...
                case DataFormat.CDAB:
                    {
                        ...
                        buffer[2] = value[index + 1];
                        buffer[3] = value[index + 0];
                        buffer[4] = value[index + 7];
                        buffer[5] = value[index + 6];
                        buffer[6] = value[index + 5];
                        buffer[7] = value[index + 4];
                        break;
                    }
                case DataFormat.DCBA:
                    {
                        ...
                        buffer[2] = value[index + 0];
                        buffer[3] = value[index + 1];
                        buffer[4] = value[index + 6];
                        buffer[5] = value[index + 7];
                        buffer[6] = value[index + 4];
                        buffer[7] = value[index + 5];
                        break;
                    }
            }
            return buffer;
